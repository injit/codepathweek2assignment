<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    if (is_blank($state['code'])) {
      ...
      $errors[] = "Country must be 2 characters.";
    }

    if (is_blank($state['country_id'])) {
      $errors[] = "Country id cannot be blank.";
    } elseif (!is_digit($state['country_id'])) {
      $errors[] = "Country id must be digit.";
    }
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    } elseif (!has_length($territory['position'], array('min' => 1, 'max' => 2)) || !is_digit($territory['position'])) {
      $errors[] = "Territiory Position must be at most 2 digits.";
    }
